...
EXP0_out,EXP0_unc = load_ensemble(EXP0,'annual_flux.csv',years)

EXP1 = glob.glob(flux_loc+'/EXP1*')
EXP1_out,EXP1_unc = load_ensemble(EXP1,'annual_flux.csv',years)
EXP1_cool_out,EXP1_cool_unc = load_ensemble(EXP1,'annual_flux_salty_cool_skin.csv',years)
...
np.savetxt('EX5_output.csv',output,delimiter=',')

for i in range(len(EXP0)):
    print(EXP0[i])
    meth= EXP0[i].split('_')[1]

    uncorr,uncorr_unc = load_ensemble([os.path.join(flux_loc,'EXP0_'+meth)],'annual_flux.csv',years)

    corr,corr_unc = load_ensemble([os.path.join(flux_loc,'EXP1_'+meth)],'annual_flux.csv',years)
    corr_cool,corr_cool_unc = load_ensemble([os.path.join(flux_loc,'EXP1_'+meth)],'annual_flux_salty_cool_skin.csv',years)
    fig = plt.figure(figsize = (7,7))
...
meth_li = ['UExP-FNN-U','sfCO2-Residual']
font = {'weight' : 'normal',
        'size'   : 16}
