...
### Running the GCB flux with a = 0.337
...
for t in head:
    ...
    plt.plot(data2['# Year'],-data2['Net air-sea CO2 flux (Pg C yr-1)'],color=cols[co],linestyle='--',label=t+' (product) ('+str(round(stat.slope*10,2))+' Pg C dec$^{-1}$) - ' + f'a = {a_scale[co]}')
    co = co+1
