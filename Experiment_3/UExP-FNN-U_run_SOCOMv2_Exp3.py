...
def download_file(ensemble,loc):
    ...
    print('Downloading...')
    print(url)
    if not du.checkfileexist(output):
...
def save_output_file(model_save_loc,ensemble,output_loc,start_yr,end_yr,version):
    c = Dataset(os.path.join(model_save_loc,'output.nc'))
    dat = c.date_file_generated
    ...
    fco2_tot = np.transpose(np.array(c['fco2_tot_unc']),[2,0,1])
    ...
    sst_o = outp.createVariable('spco2_tot_unc','f4',('time','lon','lat'),zlib=True)
    sst_o[:] = fco2_tot
    sst_o.units = 'uatm'
    sst_o.standard_name = 'Surface ocean pCO2 total uncertainty'
    sst_o.comment = "Uncertainties considered 95% confidence (2 sigma)"
...
if __name__ == '__main__':
    ...
    import datetime
    import gc
    sys.path.append(os.path.join('C:/Users/danie_em50j4m/Documents/OceanICU','Data_Loading'))
    sys.path.append(os.path.join('C:/Users/danie_em50j4m/Documents/OceanICU'))
    ...
    for i in range(len(ensembles)):
        ...
        if not os.path.exists(model_save_loc):
            ...
            for j in vars:
                act_var = np.transpose(data[j].data,(1,2,0))
                emp = np.zeros((act_var.shape[0],act_var.shape[1],1));emp[:] = np.nan
                print(act_var.shape)
                act_var = np.concatenate((emp,act_var),axis=2)
                print(act_var.shape)
                du.makefolder(os.path.join(inps,j))



                out_file= os.path.join(inps,j,'%Y_%m_'+j+'.nc')

                yr = start_yr
                mon = 1
                t = 0
                while yr <=end_yr:
                    du.netcdf_create_basic(out_file.replace('%Y',str(yr)).replace('%m',du.numstr(mon)),act_var[:,:,t],j,lat,lon)
                    mon=mon+1
                    t = t+1
                    if mon == 13:
                        yr = yr+1
                ...
                if j == 'spco2':
                    ...
                    out_file= os.path.join(inps,j,'all_'+j+'.nc')
                    direct={}
                    direct[j] = act_var
        ...
        save_output_file(model_save_loc,ensembles[i],final_output_loc,start_yr,end_yr,version)
        gc.collect()
